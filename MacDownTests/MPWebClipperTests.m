...
{
    NSString *html =
        @"<p><img src=\"/img/rete.png\"> <a href=\"../altro\">altro</a>"
        @" <a href='pagina.html'>vicino</a></p>";
    NSURL *page = [NSURL URLWithString:@"https://esempio.it/doc/rete"];

    NSString *absolute = MPHTMLWithAbsoluteAddresses(html, page);
    XCTAssertTrue([absolute containsString:@"https://esempio.it/img/rete.png"],
                  @"%@", absolute);
    XCTAssertTrue([absolute containsString:@"https://esempio.it/altro"],
                  @"%@", absolute);
    XCTAssertTrue([absolute containsString:@"https://esempio.it/doc/pagina.html"],
                  @"%@", absolute);
}

- (void)testWhatIsAlreadyAnAddressIsLeftAlone
{
    NSString *html = @"<p><a href=\"https://altrove.it/x\">fuori</a>"
                     @"<a href=\"#nota\">qui sotto</a>"
                     @"<img src=\"data:image/png;base64,AAA\"></p>";
    NSURL *page = [NSURL URLWithString:@"https://esempio.it/doc/rete"];

    NSString *absolute = MPHTMLWithAbsoluteAddresses(html, page);
    XCTAssertTrue([absolute containsString:@"https://altrove.it/x"]);
    XCTAssertTrue([absolute containsString:@"href=\"#nota\""],
                  @"un'ancora non è un altro documento");
    XCTAssertTrue([absolute containsString:@"data:image/png;base64,AAA"]);
    XCTAssertFalse([absolute containsString:@"esempio.it/doc/#nota"]);
}

- (void)testTheClippingKeepsItsPicturesReachable
{
    NSString *html = @"<html><head><title>Rete</title></head><body>"
        @"<p>Testo.</p><p><img src=\"/img/schema.png\" alt=\"schema\"></p>"
        @"</body></html>";
    NSString *clipped = MPClippedMarkdown(html,
        [NSURL URLWithString:@"https://esempio.it/doc/rete"], [NSDate date]);

    // Relative in the page, absolute in the clipping: the file lives in
    // somebody's folder now, where /img means nothing.
    XCTAssertTrue([clipped containsString:@"https://esempio.it/img/schema.png"],
                  @"%@", clipped);
}
